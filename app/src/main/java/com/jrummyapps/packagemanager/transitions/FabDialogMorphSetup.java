...
package com.jrummyapps.packagemanager.transitions;
...
public class FabDialogMorphSetup {
  ...
  /**
   * Configure the shared element transitions for morphin from a fab <-> dialog. We need to do this in code rather than
   * declaratively as we need to supply the color to transition from/to and the dialog corner radius which is
   * dynamically supplied depending upon where this screen is launched from.
   */
  public static void setupSharedEelementTransitions(@NonNull Activity activity, @Nullable View target, int radius) {
    ...
    MorphFabToDialog sharedEnter = new MorphFabToDialog(color, radius, startCornerRadius);
